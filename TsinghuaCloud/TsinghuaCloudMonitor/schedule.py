...
def test():
    ...
    for j in range(len(sort)):
        for k in range(len(sort[j]['IP'].split(';'))):
            ...
            insertData(sql3)
            sql4="insert into TsinghuaCloudMonitor_host(IP,HostName,Owner,Info,NagiosServer,HostType)value('%s','%s','%s','UP','%s','external')" % (ipgroup[k], namegroup[k], Owner[k], sort2[0]['Server'])
            insertData(sql4)
# <<<<<<< HEAD
